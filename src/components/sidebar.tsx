...
// ── Sidebar content extracted as a standalone component (fixes React #418) ──
                ...
                <li key={href}>
                  <Link href={href} onClick={onNavigate}
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
                      active ? "bg-blue-600 text-white shadow-md shadow-blue-500/25" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"
                    }`}
                  >
                ...
                <li key={href}>
                  <Link href={href} onClick={onNavigate}
                    className={`flex items-center gap-2.5 px-3 py-2 rounded-xl transition-all ${
                      active ? "bg-blue-600 text-white shadow-md shadow-blue-500/25" : "text-slate-500 hover:bg-slate-100 hover:text-slate-800"
                    }`}
                  >
...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-30" onClick={() => setOpen(false)} />
      )}
...
export default function Sidebar() {
  ...
  const SidebarContent = () => (
    <aside className="h-full w-64 bg-white border-r border-slate-200 flex flex-col">
      ...
      <nav className="flex-1 px-3 py-3 overflow-y-auto scrollbar-thin space-y-4">
        ...
        <div>
          ...
          <ul className="space-y-0.5">
            {departments.map(({ label, amharic, icon: Icon, color, text, bg }) => (
              <li key={label}>
                <button className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-slate-500 hover:bg-slate-100 hover:text-slate-800 transition-colors group">
                  <div className={`w-6 h-6 rounded-lg ${bg} flex items-center justify-center shrink-0`}>
                    <Icon size={12} className={text} />
                  </div>
                  <div className="flex-1 text-left min-w-0">
      ...
      {/* Bottom — just a thin divider line, UserButton is in the topbar */}
      <div className="px-4 py-3 border-t border-slate-100 bg-slate-50/50">
        <p className="text-[10px] text-slate-400 text-center leading-relaxed">
          Ethiopian Orthodox Tewahedo<br />University Student Association
        </p>
